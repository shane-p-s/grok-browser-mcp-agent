...
- **Granular browser (Grok)**: [`browser_granular.py`](browser_granular.py) + MCP tools **`browser_open_tab`**, **`browser_navigate`**, **`browser_get_page_state`**, **`browser_click`**, **`browser_type`**, **`browser_press_keys`** — CDP-only steps on the shared hub (no DeepSeek), each completing in seconds; optional **`screenshot_url`** per step. **`browser_open_tab`** reuses an idle tab when **`tab_label`** matches (normalized); opt out with **`reuse_existing_tab=false`**. Prefer over long **`browser_task`** when Grok’s MCP client times out.
